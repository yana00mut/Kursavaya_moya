...
try:
    ...
    logger.error("Ошибка при загрузке файла операций")
    print("Ошибка: Не удалось загрузить файл operations.xlsx")
...
def calculate_date_range(input_date):
    try:
        ...
        logger.error("Ошибка в разборе даты")
...
def filter_transactions_by_date(date_input):
    ...
    try:
        ...
        logger.error("Ошибка при фильтрации операций")
...
def find_top_transactions(operations):
    try:
        sorted_operations = []
        sorted_operations = sorted(operations, key=lambda x: x.get("Сумма операции с округлением", 0), reverse=True)
        top_transactions = sorted_operations[:5]
        return top_transactions
    except:
        logger.error("Ошибка при сортировке транзакций")
...
def fetch_currency_and_stocks(settings_path):
    ...
    try:
        ...
        logger.error("Ошибка при загрузке валют и акций")
...
def sort_transactions_by_month(transactions, date=None):
    try:
        ...
        logger.error("Ошибка при фильтрации транзакций по месяцам")
        print("Ошибка: Не удалось отфильтровать транзакции")
        return pd.DataFrame()
